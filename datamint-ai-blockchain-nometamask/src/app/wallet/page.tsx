...
import { DEMO_MODE, connectWallet, getReadContract, getWriteContract } from "@/lib/web3";
...
export default function WalletPage() {
  ...
  async function releasePayment(purchaseId: bigint) {
    ...
    try {
      ...
      setMessage(DEMO_MODE ? "Demo mode: releasing escrowed payment..." : "Opening wallet to release escrowed payment...");
      const tx = await contract.releasePayment(purchaseId);
      setMessage(DEMO_MODE ? `Demo release saved: ${tx.hash}` : `Release transaction sent: ${tx.hash}`);
      await tx.wait();
      setMessage(DEMO_MODE ? "Payment status changed to Released." : "Payment released to seller.");
      await loadWallet();
    } catch (err: any) {

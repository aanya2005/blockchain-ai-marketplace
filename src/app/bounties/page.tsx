...
export default function BountiesPage() {
  ...
  async function createBounty() {
    ...
    try {
      ...
      setStatus("Opening wallet to create bounty and lock budget in escrow...");
      ...
      setStatus(`Bounty transaction sent: ${tx.hash}`);
      await tx.wait();
      setStatus("Bounty created and budget locked on-chain.");
      await loadBounties();
    } catch (err: any) {
  ...
  async function submitToBounty() {
    ...
    try {
      const contract = await getWriteContract();
      setStatus("Opening wallet to submit your dataset CID...");
      const tx = await contract.submitBounty(submission.bountyId, submission.dataCid, submission.metadataCid, submission.note);
      setStatus(`Submission transaction sent: ${tx.hash}`);
      await tx.wait();
      setStatus("Submission recorded on-chain.");
      await loadBounties();
    } catch (err: any) {
  ...
  async function acceptFirstSubmission(bounty: Bounty) {
    ...
    try {
      ...
      setStatus("Opening wallet to accept submission and release payout...");
      const tx = await contract.acceptBountySubmission(bounty.id, firstSubmission);
      setStatus(`Accept transaction sent: ${tx.hash}`);
      await tx.wait();
      setStatus("Bounty payout released on-chain.");
      await loadBounties();
    } catch (err: any) {
